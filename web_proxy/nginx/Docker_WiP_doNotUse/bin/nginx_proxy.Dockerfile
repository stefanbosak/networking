...
# NGINX builder
#
FROM debian:stable-slim as nginx_proxy_builder
...
RUN mkdir /app /tmp/nginx /var/log/nginx /usr/share/nginx /run/nginx /var/lock/nginx /var/lib/nginx /usr/lib/nginx && \
    addgroup --system --gid 33333 nginxuser && \
    adduser --system --disabled-login --ingroup nginxuser --no-create-home --gecos "nginxuser" --shell /bin/false --uid 33333 nginxuser && \
    apt-get update && \
...
#
FROM debian:stable-slim as nginx_proxy
...
COPY --from=nginx_proxy_builder /app /
...
RUN apt-get autoremove -y && \
    ...
    ln -s /usr/sbin/nginx /app/nginx_proxy && \
    # change ownerskips to nginxuser (access for nginxuser)
    chown -R nginxuser:nginxuser /var/log/nginx \
                                 /usr/share/nginx \
                                 /run/nginx \
